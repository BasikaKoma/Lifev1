...
function MilestoneRow({
  ...
  return (
    <div
      className={`projects-row projects-row--${status.className} projects-row--major${isDragTarget ? ' projects-row--drop-target' : ''}${isDragging ? ' projects-row--dragging' : ''}`}
    >
      {draggable && (
        <button
          type="button"
          className="projects-row__drag-handle"
      ...
      <div className="projects-row__left">
        <span className="projects-row__phase">Milestone</span>
        <h3 className="projects-row__title">{stage.title}</h3>
        <p className="projects-row__desc">{stage.description}</p>
        <div className="projects-row__meta">
          <span className="projects-row__count">
      ...
      <div className="projects-row__center">
        {!isFirst && <div className="projects-row__line" aria-hidden="true" />}
        <button
          type="button"
          className={`projects-node projects-node--${status.className} projects-node--major`}
          onClick={() => onSelectStage(stage.id)}
          aria-label={`Open ${stage.title} details`}
        >
          <span className="projects-node__number">{stage.order}</span>
          <span className="projects-node__icon">{icon}</span>
        </button>
      </div>

      <div className="projects-row__right">
        <div className="projects-card projects-card--major">
          <div className="projects-card__header">
            <ProgressRing progress={progress} size={72} />
            <div className="projects-card__header-text">
              <span className="projects-card__percent">{progress}% Complete</span>
              <span className={`projects-card__health projects-card__health--${health.toLowerCase().replace(' ', '-')}`}>
                {health}
              </span>
            </div>
          </div>
          ...
          {checkpoints.length > 0 ? (
            <ul className="projects-card__checklist">
              {checkpoints.map((cp) => (
                <li
                  key={cp.id}
                  className={`projects-card__check-item ${isCheckpointDone(cp) ? 'projects-card__check-item--done' : ''}`}
                >
                  <span className="projects-card__check-icon">
                    {isCheckpointDone(cp) ? (
                      ...
                      <span className="projects-card__dot" />
                    )}
                  </span>
                  <span>{cp.title}</span>
                </li>
              ...
              {(stage.checkpoints || []).length > 5 && (
                <li className="projects-card__more">+{(stage.checkpoints || []).length - 5} more</li>
              )}
            </ul>
          ) : (
            <p className="projects-card__empty">No checkpoints yet</p>
          )}

          {(stage.ideas || []).length > 0 && (
            <p className="projects-card__ideas">{stage.ideas.length} parked idea(s)</p>
          )}

          <button type="button" className="projects-card__details" onClick={() => onSelectStage(stage.id)}>
            View details
        ...
        <div className="projects-row__connector" aria-hidden="true" />
      </div>
    </div>
  );
}
...
export function BusinessPath({
  ...
  return (
    <section className={`projects ${embedded ? 'projects--embedded' : ''} ${isActive ? 'projects--active' : ''}`}>
      <div className="projects__header">
        <div>
          <h2 className="projects__title">{embedded ? projectTitle : 'Projects'}</h2>
          <p className="projects__subtitle">
            {embedded
              ? 'Project milestones on the canvas'
              : sorted.length > 0
      ...
      {sorted.length === 0 && !showAddForm ? (
        <div className="empty-state projects-empty">
          <p>No milestones in Projects yet.</p>
          {onAddMilestone && (
